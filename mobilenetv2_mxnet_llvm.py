...
# execute
timer = m.module.time_evaluator("run", ctx, number=100, repeat=3)
prof_res = np.array(timer().results) * 1000  # convert to millisecond
print("\nMean inference time (std dev): %.2f ms (%.2f ms)" % (np.mean(prof_res), np.std(prof_res)))

tcost = timer()
std = np.std(tcost.results) * 1000
mean = tcost.mean * 1000
print("\nPerformed inference in %.2fms (std = %.2f) for %d samples" % (mean, std, 1))
print("Average per sample inference time: %.2fms" % (mean / 1)) 
...
mx.model.save_checkpoint("mobilenetv2_1.0", 0, mx_sym, args, auxs)
# there are 'resnet18_v1-0000.params' and 'resnet18_v1-symbol.json' on disk

######################################################################
# for a normal mxnet model, we start from here
mx_sym, args, auxs = mx.model.load_checkpoint("mobilenetv2_1.0", 0)
